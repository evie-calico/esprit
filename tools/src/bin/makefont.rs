...
fn main() {
	...
	if args.len() != 4 {
		eprintln!("Usage:");
		eprintln!("\t{} input_file output_file glyph_file", args[0]);
		exit(1);
	}
	...
	for ty in (0..img.height() - 1).step_by(CHARACTER_HEIGHT as usize) {
		for tx in (0..img.width()).step_by(CHARACTER_WIDTH as usize) {
			...
			output.push(size);
			glyphs += &format!("def glyph{glyph:02} = {size}\n");
			glyph += 1;
		}
	}
	...
	write(&args[3], glyphs).unwrap_or_else(|error| {
		eprintln!("Failed to write to {}: {}", args[3], error);
		exit(1);
	});
